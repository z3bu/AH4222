...
int bcmCreateDhcpCfg(char *ipAddr, char *mask,
   ...
   FILE *fd;
   //char dnsVar1[CLI_MAX_BUF_SZ], dnsVar2[CLI_MAX_BUF_SZ], str[CLI_MAX_BUF_SZ];
   char dnsVar1[CLI_MAX_BUF_SZ], dnsVar2[CLI_MAX_BUF_SZ];
...
void AutoUpgrade_GetVar(char *varName, char *varValue)
{
   IFC_AUTOUPGRADE_INFO auInfo;

   sprintf(varValue,"%s",BcmDb_getDefaultValue (varName));

   if (BcmDb_getAutoUpgradeInfo(&auInfo) == DB_GET_OK)
   {
       if (strcmp(varName,"confVersion") == 0)
       {
               sprintf(varValue, "%s", auInfo.confVersion);
       }
       else if (strcmp(varName,"downloadProtocol") == 0)
       {
               sprintf(varValue, "%s", auInfo.downloadProtocol);
       }
   }
}

void AutoUpgrade_SetVar(char *varName, char *varValue)
{
   IFC_AUTOUPGRADE_INFO auInfo;

   if (BcmDb_getAutoUpgradeInfo(&auInfo) != DB_GET_OK)
   {
       sprintf(auInfo.confVersion,"%s",BcmDb_getDefaultValue ("confVersion"));
       sprintf(auInfo.downloadProtocol,"%s",BcmDb_getDefaultValue ("downloadProtocol"));
   }

   if (strcmp(varName,"confVersion") == 0)
   {
      sprintf(auInfo.confVersion, "%s", varValue);
   }
   else if (strcmp(varName,"downloadProtocol") == 0)
   {
       sprintf(auInfo.downloadProtocol, "%s", varValue);
   }

   BcmDb_setAutoUpgradeInfo(&auInfo);
   BcmPsi_flush();
}
...
{
   if ((BcmScm_isInAccessControlList(inet_ntoa(clntAddr)) == FALSE ) && (isAccessFromLan(clntAddr) != TRUE))
...
*/
int SHA1Encription( char *input, char *output, int len )
{
    SHA1Context sha;
    int i, err;
    ...
    err = SHA1Input(&sha, (const uint8_t *) input, len);
    if (err)
    {
