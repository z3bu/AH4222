	...
	if (mask_in_addr(rt)) {
		...
		if (mask & (mask + 1)) {
			//bb_error_msg_and_die("bogus netmask %s", netmask);
                        bb_error_msg_and_die("Le masque de sous-réseau: %s est incorrecte.", netmask);
		}
		mask = ((struct sockaddr_in *) &rt.rt_dst)->sin_addr.s_addr;
		if (mask & ~mask_in_addr(rt)) {
			//bb_error_msg_and_die("netmask and route address conflict");
                        bb_error_msg_and_die("conflit d'adresse entre le masque de sous-réseau et l'adresse de routage.");
		}
	}
