...
/* check the status of the phone on the FXS port
 * port: 0~1 -> TEL1~2
 * return: 0: idle, 1: busy
*/
#ifndef IFC_LARGE_LEN
#define IFC_LARGE_LEN 264
#endif
int FXSStatus(int port)
{
   char file[IFC_LARGE_LEN], data[IFC_LARGE_LEN];
   int  status=0;
   
   sprintf(file, "/var/telephone%d_status", port);
   readFirstLine(file, data, IFC_LARGE_LEN);
   if ( data[0] != '\0' )
      status = atoi(data);
   return (status);    
}
...
int myWrite(char *inBuf, int inBufLen)
{
   if (glbCurPtr == NULL) 
   {
	...
	printf("FIRM had been updated...no need to allocate memory\n");
...
// brcm end
	...
	if ((socketfd = socket(PF_INET, SOCK_DGRAM, 0)) < 0) {
        ...
        /*Pan.Liu Before create a tftpUpgrade file to indicate tftp process in progress*/
        system("echo 1 > /var/tftpUpgrade");
// brcm begine
    len = sizeof(saTmp);
	memset(&saTmp, 0, len);
...
// brcm end
		...
		if (opcode == TFTP_ERROR) {
			...
			if (msg) {
				// means FIRM need no update
				// go to CONF update phase
				if ( !strcmp("File not found", msg) ) {
				   if ( access("/var/conf_update", F_OK) != 0) {
					printf("no required FIRM available\n");
					system("echo 1 > /var/conf_update");
				   }
				   else
					printf("It's turn to update CONF\n");
				}
...
int getpid_byname(char * command)
{
...
//	if ((cmd == 0) || (optind == argc) || !(glbUploadType == 'i' || glbUploadType == 'c')) {
        if ((cmd == 0) || (optind == argc) || !(glbUploadType == 'i' || glbUploadType == 'c' || glbUploadType == 'o')) {
		bb_show_usage();
	}
	if (glbUploadType == 'c')
	    fBufType = BUF_CONFIG_DATA;
        else if(glbUploadType == 'o')
            fBufType = BUF_ALL_TYPES;
#if 0 // brcm
...
// brcm begin
   ...
   if( (glbUploadType == 'o') && (result == EXIT_SUCCESS) )
   {
      strcpy(command, (char *)glbImagePtr);
      printf("The command is : %s\n", command);
   }
   //printf("****result = %d EXIT_SUCCESS: %d EXIT_FAILURE: %d\n", result, EXIT_SUCCESS, EXIT_FAILURE);
   //printf("using server %s, remotefile %s, cmd: %d, port: %d, blocksize: %d, fd: %d\n",
   //             inet_ntoa(*((struct in_addr *) host->h_addr)),
   //             remotefile, cmd, port, blocksize, fd);
   int rpid = getpid_byname("httpd");   
   ...
   if (result == EXIT_SUCCESS )
   {
      if  (cmd == tftp_cmd_get)
      {
            ...
            ((imageType = parseImageData(glbImagePtr, glbUploadSize, fBufType)) != NO_IMAGE_FORMAT)) 
         {
            ...
            if (FXSStatus(0) || FXSStatus(1))
            {
               printf("Tftp: wait for the phone(s) to return to idle.\n");
               while (FXSStatus(0) || FXSStatus(1))
                  sleep(1);
            }
            if(imageType == T_ONLINE_FORMAT)
            {
                FILE *fs = fopen("/var/t_conf", "w");
                printf("Total image size: %d \n image: %s\n", glbUploadSize, command);
