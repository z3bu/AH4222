...
static int flash_wait_erase(WORD sector, int offset, UINT16 data); //pan for MXIC flash erase operation polling
...
static int flashPrg=0;
static struct cfi_query query;
void Delay_1_Micro_Seconds(void);        // delay 20us  Pan.Liu
...
void     Delay_1_Micro_Seconds(void)      // delay 20us.
         { int i;
		   for(i=0; i<70; i++);        // change the number according to your system speed.
          }
