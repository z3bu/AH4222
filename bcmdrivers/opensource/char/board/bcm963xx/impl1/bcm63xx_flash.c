...
PFILE_TAG kerSysImageTagGet(void)
{
    ...
    {
        sectAddr =  flash_get_memptr((unsigned short) i);
...
//        }
 	...
 	#ifdef memcheck   //added by Pan.Liu to retry three times
	for(j=0;j<3;j++)
	{
        #endif
//        if ((i = flash_write_buf(blk_start, 0, string, sect_size)) != sect_size) {
  //          break;
	...
	if ((i = flash_write_buf(blk_start, 0, string, sect_size)) == sect_size)
	 ...
	 {
           printk("Enter memcheck loop.-- BCM Image set\n");  
	   printk("Flash sector number:%d\n",blk_start);
	   printk("flash write:%d sect_size:%d\n",i,sect_size);
	   printk("Error occur flash write!!\n...enter rewrite process  #:%d !\n",j);
	 }
	}	  
        if(j==2 && i!= sect_size)
	   break;
